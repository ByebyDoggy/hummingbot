...
class XEMMMultipleLevelsConfig(ControllerConfigBase):
    ...
    maker_connector_leverage: int = Field(
        default=1,
        json_schema_extra={"prompt": "Enter the maker connector leverage: ", "prompt_on_new": True})
    ...
    taker_connector: str = Field(
        default="okx_perpetual",
        json_schema_extra={"prompt": "Enter the taker connector: ", "prompt_on_new": True})
    taker_connector_leverage: int = Field(
        default=1,
        json_schema_extra={"prompt": "Enter the taker connector leverage: ", "prompt_on_new": True})
        ...
        for target_profitability, amount in self.buy_levels_targets_amount:
            ...
            if len(active_buy_executors_target) == 0 and imbalance < self.config.max_executors_imbalance:
                ...
                config = XEMMExecutorConfig(
                    ...
                    taker_leverage=self.config.taker_connector_leverage,
                    maker_leverage=self.config.maker_connector_leverage
                )
